...
凭据从 settings.toml 读 (wechat_app_id / wechat_app_secret), 运营凭据
按站点不同, 即时生效。凭据未配置时返回空串, 不注入 JS, 不报错。
...
class WeChatShare:
    ...
    _JS_TEMPLATE = Template("""<script type="text/javascript" src="https://res.wx.qq.com/open/js/jweixin-1.6.0.js"></script>
<script>
wx.config({
    debug: false,
    appId: "{{ appid }}",
...
wx.ready(function () {
    ...
    def __init__(self, request: Request, *api_list: str) -> None:
        ...
        self.api_list = (
            list(api_list)
            if api_list
            else ["updateAppMessageShareData", "updateTimelineShareData"]
        )
        self.appid = get_settings("wechat_app_id", "")
        self.app_secret = get_settings("wechat_app_secret", "")
        self.nonce_str = "".join(
    ...
    async def render(
        self, title: str = "", desc: str = "", link: str = "", img: str = ""
    ) -> str:
        """渲染分享 JS。凭据未配置返回空串, 不注入。"""
        if not self.appid or not self.app_secret:
            return ""
        return self._JS_TEMPLATE.render(
            appid=self.appid,
