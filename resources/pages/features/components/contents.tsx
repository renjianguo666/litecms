...
import { feature, article } from "@/lib/api";
...
interface FeatureContentsProps {
  featureId: string;
}

export default function FeatureContents(props: FeatureContentsProps) {
  // 状态
  ...
  const handleRemove = async (contentId: string) => {
    try {
      await feature.removeContent(props.featureId, contentId);
      toast.success("移除成功");
      refetch();
    } catch {
      toast.error("移除失败");
  ...
  const handleAddSelected = async () => {
    ...
    setIsAdding(true);
    try {
      const result = await feature.addContents(props.featureId, ids);
    ...
    <div class="space-y-4">
        ...
        <Show
          ...
          fallback={
            <div class="text-center py-12 text-muted-foreground">
              ...
              <p class="text-sm">点击「添加文章」将文章加入此推荐位</p>
            </div>
          }
        >
      ...
      <Dialog open={showAddModal()} onOpenChange={setShowAddModal}>
        <DialogContent class="max-w-2xl">
          <DialogHeader>
            <DialogTitle>添加文章到推荐位</DialogTitle>
          </DialogHeader>
